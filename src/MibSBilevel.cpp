			   ...
			   MibSModel *mibs)
{
  ...
  int lRowNum(model_->getLowerRowNum());
  ...
  MibSAddTenderVars useTenderVars = static_cast<MibSAddTenderVars>
      (model_->MibSPar_->entry(MibSParams::useTenderVars));
  ...
  if(useTenderVars == MibSAddTenderVarsIsAllowed){
      isTenderVarsFixed_ = true;
      for(i = N - lRowNum; i < N; i++){
	  if(fabs(upper[i] - lower[i]) > etol){
	      isTenderVarsFixed_ = false;
	      break;
	  }
      }
  ...
  //sahar: check if the following part is only
  //necessary when use linkingPool
			      ...
			      bool newOsi, const double *lpSol)
{
	...
	if((isTenderVarsFixed_ == false) && (fixedInd[index1] == 1)){
		colLb[index1] = floor(lpSol[index1] + 0.5);
		colUb[index1] = colLb[index1];
	    }
	else if((isTenderVarsFixed_ == true) && (index1 >= colNum - lRows)){
	    colLb[index1] = floor(lpSol[index1] + 0.5);
	    colUb[index1] = colLb[index1];
	}
    }
