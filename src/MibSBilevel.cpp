			...
			bool newOsi, const double *lpSol)
{
  ...
  // number of upper-level rows with lower-level variables
  int uNum(0);
  ...
  if (newOsi){
    ...
    if(!bestSolLL){
	 ...
	 rowLb[rowNum-1] = oSolver->getInfinity();
	 rowLb[rowNum-1] *= -1;
     }
...
#define SYM_VERSION_IS_WS strcmp(SYMPHONY_VERSION, "WS")  
  ...
  if (feasCheckSolver == "SYMPHONY" && probType == 1 && warmStartLL &&
	...
	nSolver->setColUpper(i, newUbVal[i]);
     ...
     double * upComp = new double[lRows+uNum];
     CoinFillN(upComp, lRows+uNum, 0.0);
